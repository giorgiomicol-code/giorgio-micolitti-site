...
- [ ] User raised the idea of making each publication title link to the actual PDF (opening in a centered overlay, page-flip style — referencing a "Villabrando/Welcome Book" project this session has no context on) and, if that's in place, possibly skipping cover thumbnails altogether. Deferred per the user ("poi vediamo") — needs the actual PDF files of each publication before this can be built at all.
