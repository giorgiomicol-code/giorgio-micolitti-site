...
- [x] **PDF flipbook viewer built and wired to all 7 publications.** User uploaded 11 PDF files to the repo root; identified each by rendering and reading its first page with PyMuPDF (not by filename alone) and matched 7 of them to the CV's 7 publications with visual confirmation (author byline, magazine masthead, matching title). Moved the 7 into `public/publications/` with clean filenames.
  ...
  - **Not published**: `Giorgio MICOLITTI- Manuale Di Progettazione Gallerie RFI.pdf` (172 pages) — its own first page states *"RFI si riserva la proprietà di questo documento che non potrà essere copiato, riprodotto o comunicato ad altri senza esplicita autorizzazione"*. Left out of `public/publications/` and off the site entirely pending explicit confirmation that the user has RFI's authorisation to distribute it.
...
- [ ] **"Manuale Di Progettazione Gallerie RFI.pdf"**: not published — see above. Confirm whether you have RFI's authorisation to distribute it before it's added anywhere.
