...
function App() {
  ...
  return <div className="site-shell">
    ...
    <main id="top">
      ...
      <section id="results" className="light"><div className="wrap"><div className="results-panel"><div className="section-head"><p className="kicker">05 · {lang === 'it' ? 'ESPERIENZE IN NUMERI' : 'EXPERIENCE IN NUMBERS'}</p><h2>{lang === 'it' ? 'Esperienze in numeri' : 'Experience in numbers'}</h2></div><div className="results-grid">{results.map((x,i)=>{const [value,label]=t(x).split(' · ');return <article key={i}><span className="stat-value">{value}</span><span className="stat-label">{label}</span></article>})}</div></div></div></section>
