...
function App() {
  ...
  return <div className="site-shell">
    <header className="site-header"><a className="brand" href="#top"><span className="brand-mark">GM</span><span>Giorgio Micolitti</span></a><button type="button" className={navOpen ? 'nav-toggle nav-toggle-open' : 'nav-toggle'} aria-expanded={navOpen} aria-label={lang === 'it' ? 'Apri il menu' : 'Open menu'} onClick={() => setNavOpen(o => !o)}><span/><span/><span/></button><nav className={navOpen ? 'nav-open' : ''} onClick={() => setNavOpen(false)}><a href="#profile">{lang === 'it' ? 'Profilo' : 'Profile'}</a><a href="#rfi">{lang === 'it' ? '25 anni RFI' : '25 years RFI'}</a><a href="#diary">Project diary</a><a href="#bridge">{lang === 'it' ? 'Ponte' : 'Bridge'}</a><a href="#teaching">{lang === 'it' ? 'Docenze' : 'Teaching'}</a><a href="#publications">{lang === 'it' ? 'Pubblicazioni' : 'Publications'}</a></nav><div className="lang"><button className={lang === 'it' ? 'active' : ''} onClick={() => setLang('it')}>IT</button><span>/</span><button className={lang === 'en' ? 'active' : ''} onClick={() => setLang('en')}>EN</button></div></header>
    <main id="top">
