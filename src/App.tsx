...
function VideoPlayer({ id, title, code }: { id: string; title: string; code?: string }) {
  ...
  return playing
    ...
    : <button type="button" className="video-player" aria-label={title} onClick={() => setPlaying(true)}>
        ...
        {code && <span className="video-code-badge">{code}</span>}
...
function HeroVisual() {
  ...
  return <figure className="hero-band">
    {broken
      ? <p className="placeholder">IMMAGINE CANTIERE<br/>in attesa del file head_foto_1584x396.webp</p>
      : <img src="./images/head_foto_1584x396.webp" alt="Cantieri ferroviari" loading="lazy" onError={() => setBroken(true)}/>}
  </figure>
}
...
function DiaryPhotoGrid({ images, lang, tag }: { images: string[]; lang: Lang; tag?: string }) {
  ...
  const renderFigure = (img: string, i: number) => {
    ...
    return <figure key={img}><span className="diary-code">{diaryCodes[img]}</span><button type="button" className="diary-zoom" onClick={() => setOpenIndex(i)} aria-label={lang === 'it' ? 'Ingrandisci' : 'Enlarge'}><img src={`./images/${d.image}`} alt={t(d.title)} loading="lazy"/></button><figcaption>{tag && <span className="diary-tag">{tag}</span>}{t(d.title)}</figcaption></figure>
  }
...
function isLastGridRow(total: number, i: number, cols = 3) {
    ...
    {items.map((e, i) => {
      ...
      return <div className="exp-item" key={i}>
        ...
        <button type="button" className="exp-trigger" aria-expanded={isOpen} aria-controls={panelId} onClick={() => toggle(i)}>
          <span className="exp-meta"><span className="exp-years">{e.years} · {e.org}</span><span className={`exp-chevron${isOpen ? ' exp-chevron-open' : ''}`} aria-hidden="true"><svg viewBox="0 0 24 24" width="11" height="11"><polygon points="16,5 16,19 6,12" fill="currentColor"/></svg></span></span>
          <h4>{t(e.role)}</h4>
        </button>
...
const profileIntro: Copy[] = [
  { it: 'Ingegnere Civile Geotecnico, Dirigente e Project & Construction Manager, con oltre 24 anni di esperienza nella progettazione, realizzazione e gestione di grandi infrastrutture ferroviarie.', en: 'Civil & Geotechnical Engineer, Executive and Project & Construction Manager, with over 24 years of experience in the design, delivery and management of major railway infrastructure.' },
...
const experience: ExperienceItem[] = [
  { years: '2023 — Presente', role: { it: 'Responsabile Ingegneria Collegamenti Ferroviari', en: 'Head of Railway Connections Engineering' }, org: 'Stretto di Messina S.p.A. · Roma', text: [
  ...
  { years: '2017 — 2023', role: { it: 'Responsabile Ingegneria Civile, Armamento, Patrimonio e Autorizzazioni, Ponti e Verifiche', en: 'Head of Civil Engineering, Trackwork, Property & Authorisations, Bridges & Inspections' }, org: 'RFI S.p.A. · DOIT Roma', text: [
  ...
  { years: '2016 — 2017', role: { it: 'Responsabile Opere Civili, Fabbricati e Impianti Speciali', en: 'Head of Civil Works, Buildings and Special Systems' }, org: 'RFI S.p.A. · DOIT RM', text: [
  ...
  { years: '2014 — 2016', role: { it: 'Responsabile Team Sviluppo Locale Opere Civili', en: 'Head of Local Development Civil Works Unit' }, org: 'RFI S.p.A. · DOIT RM', text: [
  ...
  { years: '2011 — 2014', role: { it: 'Project Manager e Referente Task Force Sicurezza nelle Gallerie', en: 'Project Manager and Tunnel Safety Task Force Representative' }, org: 'RFI S.p.A. · DOIT RM', text: [
  ...
  { years: '2009 — 2011', role: { it: 'Referente Pianificazione Industriale e Monitoraggio KPI – Business Immobiliare e Trasporto', en: 'Industrial Planning & KPI Monitoring – Real Estate and Transport Business' }, org: 'FS S.p.A. · Direzione Centrale Strategie', text: [
  ...
  { years: '2006 — 2009', role: { it: 'Ingegnere di Progetto – Opere Civili, Strutture in Sotterraneo e Sicurezza', en: 'Project Engineer – Civil Works, Underground Structures and Safety' }, org: 'RFI S.p.A. · Direzione Tecnica, Ingegneria Civile', text: [
  ...
  { years: '2002 — 2006', role: { it: 'Ingegnere di Progetto – Progettazione e Pianificazione Infrastrutture', en: 'Project Engineer – Infrastructure Design and Planning' }, org: 'RFI S.p.A. · Direzione Investimenti', text: [
  ...
  { years: '2001 — 2002', role: { it: 'Ingegnere di Progetto', en: 'Project Engineer' }, org: 'DMS Engineering S.r.l. · Progettazione Geotecnica e Strutturale', text: [
  ...
  { years: '2000 — 2001', role: { it: 'Ingegnere di Progetto – Area Tecnico-Patrimoniale', en: 'Project Engineer – Technical Property Area' }, org: 'ASL Latina', text: [
...
const infraDomains: AreaItem[] = [
  ...
  { title: { it: 'Grandi programmi infrastrutturali', en: 'Major infrastructure programmes' }, detail: { it: 'Reti TEN-T · Interoperabilità · Collegamenti ferroviari · Opere transfrontaliere', en: 'TEN-T networks · Interoperability · Railway connections · Cross-border works' } },
]

const crossCompetencies: AreaItem[] = [
  { title: { it: 'Project & Construction Management', en: 'Project & Construction Management' }, detail: { it: 'Progettazione · Pianificazione · Tempi, costi e qualità · Realizzazione', en: 'Design · Planning · Schedule, cost and quality · Delivery' } },
  { title: { it: 'Sicurezza e gestione del rischio', en: 'Safety and risk management' }, detail: { it: 'Sicurezza in galleria · Risk management · Rischio sismico e idrogeologico', en: 'Tunnel safety · Risk management · Seismic and hydrogeological risk' } },
  { title: { it: 'Verifica, collaudo e controllo tecnico', en: 'Testing, commissioning and technical review' }, detail: { it: 'Collaudi statici · Collaudi tecnico-amministrativi · Verifiche progettuali', en: 'Structural testing · Technical-administrative testing · Design verification' } },
  ...
  { title: { it: 'Coordinamento multidisciplinare', en: 'Multidisciplinary coordination' }, detail: { it: 'Team progettisti · Imprese · Enti e stakeholder', en: 'Design teams · Contractors · Public bodies and stakeholders' } },
...
const diary: DiaryItem[] = [
  ...
  { image: 'galleria-scavo-fresa.webp', title: { it: 'Ricostruzione dell’anello di rivestimento a seguito del cedimento strutturale della sezione centrale galleria', en: 'Reconstruction of the lining ring following the structural failure of the tunnel’s central section' }, text: { it: 'Avanzamento dello scavo con fresa meccanizzata.', en: 'Excavation advance using a tunnel boring machine.' } },
  ...
  { image: 'galleria-interno-02.webp', title: { it: 'Fasi di realizzazione dei condotti aeraulici. Fase 3: posa dei condotti aeraulici per la messa in sovrappressione delle zone filtro', en: 'Construction phases of the ventilation ducts. Phase 3: installation of the ventilation ducts for pressurising the filter zones' }, text: { it: 'Attività di manutenzione e controllo delle strutture in sotterraneo.', en: 'Maintenance and structural monitoring activities underground.' } },
  { image: 'galleria-interno-scavo.webp', title: { it: 'Ultimazione degli impianti', en: 'Completion of the systems' }, text: { it: 'Fase di avanzamento al fronte durante lo scavo in sotterraneo.', en: 'Face-advance phase during underground excavation.' } },
  { image: 'galleria-percorrenza.webp', title: { it: 'Predisposizioni per la posa dell’impiantistica di emergenza', en: 'Preparatory works for the installation of the emergency systems' }, text: { it: 'Ispezione tecnica lungo lo sviluppo della galleria.', en: 'Technical inspection along the length of the tunnel.' } },
  { image: 'galleria-prima-dopo-collage.webp', title: { it: 'Ante e post intervento di consolidamento', en: 'Before and after the strengthening works' }, text: { it: 'Confronto tra lo stato ante operam e post operam di un intervento in galleria.', en: 'Comparison between the pre- and post-intervention state of a tunnel upgrade.' } },
  ...
  { image: 'vallone-scarpa-viadotto.jpg', title: { it: 'Vallone, scarpa del viadotto', en: 'Vallone, viaduct slope' }, text: { it: 'Vallone, scarpa del viadotto', en: 'Vallone, viaduct slope' } },
  { image: 'demolizione-ponte.jpg', title: { it: 'Demolizione ponte', en: 'Bridge demolition' }, text: { it: 'Demolizione ponte', en: 'Bridge demolition' } },
  { image: 'galleria-orte-1.webp', title: { it: 'Rimozione di un arco rovescio', en: 'Removal of an inverted arch' }, text: { it: 'Rimozione di un arco rovescio', en: 'Removal of an inverted arch' } },
  { image: 'galleria-orte-2.webp', title: { it: 'Ricostruzione di un arco rovescio mediante elementi prefabbricati in acciaio', en: 'Reconstruction of an inverted arch using precast steel elements' }, text: { it: 'Ricostruzione di un arco rovescio mediante elementi prefabbricati in acciaio', en: 'Reconstruction of an inverted arch using precast steel elements' } },
  { image: 'galleria-orte-3.webp', title: { it: 'Completamento e ultimazione dei lavori di ricostruzione dell’arco rovescio', en: 'Completion of the inverted arch reconstruction works' }, text: { it: 'Completamento e ultimazione dei lavori di ricostruzione dell’arco rovescio', en: 'Completion of the inverted arch reconstruction works' } },
...
const diaryTaxonomy: DiaryCategory[] = [
  ...
  { title: { it: 'Stazioni e ambito urbano', en: 'Stations and urban areas' }, subcategories: [
    { title: { it: 'Cantieri urbani', en: 'Urban construction sites' }, images: ['cantiere-vista-dal-basso.webp', 'cantiere-urbano-02.webp', 'cantiere-impalcatura-verticale.webp', 'cantiere-urbano-01.webp'] },
...
// Codice lettera+numero univoco per OGNI video del sito, nell'ordine in cui compaiono
// in pagina (25 anni RFI, Video & Insights, Agosto 2022, Ponte, Docenze). Usato solo come
// riferimento univoco in conversazione: non rinomina alcun file, la V sta per "video".
const allVideoIds = ['9LUx1LBOJzA', ...videos.map(v => v.id), 'B2XEBK2oEao', '8hU4DBr4O5s', 'iwm4JwfobGA', 'VoLegEc26A8', '5Zk6x1oAifc']
const videoCodes: Record<string, string> = Object.fromEntries(allVideoIds.map((id, i) => [id, `V${i + 1}`]))
...
function App() {
  ...
  return <div className="site-shell">
    <header className="site-header"><a className="brand" href="#top"><span className="brand-mark">GM</span><span>Giorgio Micolitti</span></a><button type="button" className={navOpen ? 'nav-toggle nav-toggle-open' : 'nav-toggle'} aria-expanded={navOpen} aria-label={lang === 'it' ? 'Apri il menu' : 'Open menu'} onClick={() => setNavOpen(o => !o)}><span/><span/><span/></button><nav className={navOpen ? 'nav-open' : ''} onClick={() => setNavOpen(false)}><a href="#profile">{lang === 'it' ? 'Profilo' : 'Profile'}</a><a href="#rfi">{lang === 'it' ? '25 anni RFI' : '25 years RFI'}</a><a href="#diary">Project diary</a><a href="#bridge">{lang === 'it' ? 'Ponte sullo Stretto' : 'Strait of Messina'}</a><a href="#teaching">{lang === 'it' ? 'Docenze' : 'Teaching'}</a><a href="#publications">{lang === 'it' ? 'Pubblicazioni' : 'Publications'}</a></nav><div className="lang"><button className={lang === 'it' ? 'active' : ''} onClick={() => setLang('it')}>IT</button><span>/</span><button className={lang === 'en' ? 'active' : ''} onClick={() => setLang('en')}>EN</button></div></header>
    <main id="top">
      ...
      <section id="videos" className="dark"><div className="wrap section-head"><p className="kicker">07 · {lang === 'it' ? 'VIDEO & INSIGHTS' : 'VIDEO & INSIGHTS'}</p><h2>{lang === 'it' ? 'Vari, rimozioni e opere civili in sequenza' : 'Launches, removals and civil works in sequence'}</h2></div><div className="wrap video-grid">{videos.map(v=><article className="video-card" key={v.id}><VideoPlayer id={v.id} title={t(v.title)} code={videoCodes[v.id]}/><div className="video-caption-head"><h3>{t(v.title)}</h3></div><p>{t(v.caption)}</p></article>)}</div></section>
      <section id="august-2022" className="editorial light"><div className="wrap editorial-grid"><div><p className="kicker">08 · {lang === 'it' ? 'AGOSTO 2022' : 'AUGUST 2022'}</p><h2>{lang === 'it' ? 'Il suono di due treni su un’opera appena realizzata' : 'The sound of two trains on a newly completed civil work'}</h2></div><div><p className="lead">{lang === 'it' ? 'Uno dei lavori estivi eseguiti dalla squadra di Ingegneria Civile della DOIT Roma, in finestre operative complesse e programmate nei mesi di minore traffico.' : 'One of the summer works delivered by the DOIT Rome Civil Engineering team within complex possessions planned during lower-traffic periods.'}</p><div className="video-embed"><VideoPlayer id="B2XEBK2oEao" title={lang === 'it' ? 'Agosto 2022' : 'August 2022'} code={videoCodes['B2XEBK2oEao']}/></div></div></div></section>
      <section id="bridge" className="bridge dark"><div className="wrap bridge-grid"><div><p className="kicker">09 · {lang === 'it' ? 'PONTE SULLO STRETTO' : 'STRAIT OF MESSINA BRIDGE'}</p><h2>{lang === 'it' ? 'Collegamenti ferroviari del Ponte sullo Stretto' : 'Railway connections to the Strait of Messina Bridge'}</h2></div><div><p className="lead">{lang === 'it' ? 'Dal novembre 2023 Giorgio Micolitti è Responsabile Ingegneria Collegamenti Ferroviari di Stretto di Messina S.p.A. e guida la relativa Struttura Organizzativa.' : 'Since November 2023, Giorgio Micolitti has served as Head of Railway Connections Engineering at Stretto di Messina S.p.A., leading the related organisational unit.'}</p><p>{lang === 'it' ? 'La sezione mantiene volutamente separati ruolo professionale e documentazione fotografica: vengono utilizzate soltanto immagini con attribuzione certa.' : 'The section deliberately keeps professional role and photographic documentation distinct: only imagery with certain attribution is used.'}</p></div></div><div className="wrap bridge-videos"><div className="video-embed"><VideoPlayer id="8hU4DBr4O5s" title={lang === 'it' ? 'Ponte sullo Stretto di Messina — Collegamenti Ferroviari' : 'Strait of Messina Bridge — Railway Connections'} code={videoCodes['8hU4DBr4O5s']}/><p className="video-caption">{lang === 'it' ? 'Ponte sullo Stretto di Messina: Collegamenti Ferroviari a terra e effetti sulle preesistenze. Messina, 29 ottobre 2024.' : 'Strait of Messina Bridge: land-side railway connections and effects on existing assets. Messina, 29 October 2024.'}</p></div><div className="video-embed"><VideoPlayer id="iwm4JwfobGA" title={lang === 'it' ? 'Esplorando il Futuro dei Collegamenti Ferroviari al Ponte sullo Stretto' : 'Exploring the Future of Railway Connections to the Strait of Messina Bridge'} code={videoCodes['iwm4JwfobGA']}/><p className="video-caption">{lang === 'it' ? 'Esplorando il Futuro dei Collegamenti Ferroviari al Ponte sullo Stretto di Messina. Messina, 18 marzo 2024.' : 'Exploring the Future of Railway Connections to the Strait of Messina Bridge. Messina, 18 March 2024.'}</p></div></div></section>
      <section id="teaching" className="light"><div className="wrap teaching-grid"><p className="kicker">10 · {lang === 'it' ? 'DOCENZE' : 'TEACHING'}</p><h2>Sapienza<br/>{lang === 'it' ? 'Università di Roma' : 'University of Rome'}</h2><p className="lead">{lang === 'it' ? 'Docente di Tecnica dei Cantieri Infrastrutturali presso il Master Sapienza in Ingegneria delle Infrastrutture e dei Sistemi Ferroviari (IIS), edizioni 2020, 2021 e 2022.' : 'Lecturer in Infrastructure Construction Site Techniques within Sapienza University Master in Infrastructure and Railway Systems Engineering (IIS), editions 2020, 2021 and 2022.'}</p><div className="bridge-videos"><div className="video-embed"><VideoPlayer id="VoLegEc26A8" title={lang === 'it' ? 'Lezione Master IIS' : 'Master IIS Lecture'} code={videoCodes['VoLegEc26A8']}/><p className="video-caption">{lang === 'it' ? 'Lezione Master IIS del 10.06.2022 — Master Università Sapienza di Roma, Ingegneria delle Infrastrutture e dei Sistemi Ferroviari, ed. 2022.' : 'Master IIS lecture, 10 June 2022 — Sapienza University of Rome Master in Infrastructure and Railway Systems Engineering, 2022 edition.'}</p></div><div className="video-embed"><VideoPlayer id="5Zk6x1oAifc" title={lang === 'it' ? 'Docenza di Tecnica dei Cantieri Infrastrutturali al Master IIS 2021' : 'Lecture on Infrastructure Construction Site Techniques at the IIS Master, 2021'} code={videoCodes['5Zk6x1oAifc']}/><p className="video-caption">{lang === 'it' ? 'Docenza di Tecnica dei Cantieri Infrastrutturali al Master IIS 2021.' : 'Lecture on Infrastructure Construction Site Techniques at the IIS Master, 2021 edition.'}</p></div></div></div></section>
      <section id="education" className="blue-light"><div className="wrap section-head"><p className="kicker">11 · {lang === 'it' ? 'FORMAZIONE' : 'EDUCATION'}</p><h2>{lang === 'it' ? 'Percorso accademico e abilitazione professionale' : 'Academic background and professional licence'}</h2></div><div className="wrap education-list">{education.map((e,i)=><article className="experience-item" key={i}><span>{e.years}</span><h4>{t(e.title)}</h4><strong>{t(e.org)}</strong><p>{t(e.detail)}</p></article>)}</div></section>
      <section id="publications" className="publications ivory">{pubRows.map((row,ri)=><div key={ri} id={ri === 0 ? 'publications-grid' : undefined} className={`pub-band ${ri % 2 === 0 ? 'pub-band-navy' : 'pub-band-white'}`}>{ri === 0 && <div className="wrap section-head"><p className="kicker">12 · {lang === 'it' ? 'PUBBLICAZIONI SELEZIONATE' : 'SELECTED PUBLICATIONS'}</p><h2>{lang === 'it' ? 'Ricerca, sicurezza, normativa e geotecnica' : 'Research, safety, regulation and geotechnics'}</h2></div>}<div className="wrap pub-grid">{row.map((p,i)=><PubCard key={i} p={p} code={pubCodes[ri*4+i]} lang={lang} t={t}/>)}</div></div>)}<div className="wrap awards"><h3>{lang === 'it' ? 'Premi e riconoscimenti' : 'Awards and recognition'}</h3>{awards.map(a=><div className="award-row" key={a.year}><div className="award-head"><strong>{a.year}</strong><span>{t(a.text)}</span>{a.photos && <AwardPhotos photos={a.photos} alt={t(a.text)} lang={lang}/>}</div>{a.detail && lang === 'it' && <div className="award-detail">{a.detail.map((p,i)=><p key={i}>{p}</p>)}</div>}</div>)}</div>{documents.length > 0 && <div className="wrap documents">{documents.map((d,i)=><figure key={i}><img src={`./images/${d.image}`} alt={t(d.title)} loading="lazy"/><figcaption>{t(d.title)}</figcaption></figure>)}</div>}</section>
